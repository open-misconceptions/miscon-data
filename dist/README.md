...
Generated distribution files. Regenerate with `miscon export all`; committed
...
| File | What |
|------|------|
| `miscon.case.json` | 1EdTech CASE 1.1 `CFPackage`: one `CFDocument`, a `Domain` `CFItem` per top-level domain, a `Misconception` `CFItem` per record, `isChildOf` associations record → domain → document, and `isRelatedTo` associations for every alignment with a URI and every `relations.*` target. |
| `miscon.jsonl` | One record per line, exactly as in `records/` minus `$schema`. |
| `miscon.csv` | One row per record with the scalar fields and the first evidence example. |

## CASE mapping

| Open Misconceptions | CASE |
|-----|------|
| library | `CFDocument` (`title`, `creator`, `licenseURI` → CC BY 4.0, `version` = release tag) |
| record `uuid` | `CFItem.identifier` |
| `miscon:<id>` | `CFItem.humanCodingScheme` |
...
`miscon export case` runs a structural check (`tools/miscon/miscon/case_check.py`)
