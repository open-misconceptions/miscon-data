...
`miscon validate` proves the result (it already requires `uri == base + "/m/" + id`).
...
What it does NOT change: the `miscon:` IDs themselves. Only the host that resolves
them moves.
"""
...
def _rewrite_scheme_patterns(path: Path, old_base: str, new_base: str, dry_run: bool) -> int:
    ...
    The pattern stores the host regex-escaped (`old\\.example\\.org`), so a plain
    text replacement of the unescaped base misses it and leaves the Miscon scheme
    matching the old host — which surfaces as a warning on every record with an
    Miscon concept URI, and blocks CI under --strict. Rewriting the parsed value
