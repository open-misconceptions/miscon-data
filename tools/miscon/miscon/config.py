...
def find_repo_root(start: Path | None = None) -> Path:
    """Walk up from `start` (default: cwd) until a directory holding miscon.config.json."""
    here = (start or Path.cwd()).resolve()
    for candidate in [here, *here.parents]:
        if (candidate / "miscon.config.json").is_file():
            return candidate
    raise FileNotFoundError("miscon.config.json not found in this directory or any parent")
...
class Config:
    ...
    def load(cls, root: Path | None = None) -> "Config":
        ...
        raw = json.loads((root / "miscon.config.json").read_text(encoding="utf-8"))
        ...
        return cls(
            ...
            title=raw.get("title", "Open Misconceptions"),
            ...
            schema_root=Path(os.environ["MISCON_SCHEMA_DIR"]) if os.environ.get("MISCON_SCHEMA_DIR") else None,
        )
