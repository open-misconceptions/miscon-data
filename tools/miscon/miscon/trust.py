...
def reviewer_weights(registry: dict) -> dict[str, float]:
    """Every id in the registry, plus every alias, mapped to its owner's weight.

    A reviewer's durable handle can change -- a GitHub handle gives way to an
    ORCID -- and reviews already recorded under the old one must keep counting.
    `aliases` is what makes that true; without it, promoting a handle silently
    drops every review filed under the previous one.
    """
    weights: dict[str, float] = {}
    for reviewer in registry.get("reviewers", []):
        weight = float(reviewer.get("weight", 0))
        weights[reviewer["id"]] = weight
        for alias in reviewer.get("aliases", []) or []:
