...
def build_case(config: Config, records: list[dict], version: str | None = None) -> dict:
    ...
    cf_document = {
        ...
        "description": (
            ...
            "misconception; humanCodingScheme carries the miscon: identifier."
        ),
        ...
        "notes": f"Generated from records/ by miscon export case. {len(records)} records.",
        ...
        cf_items.append(
            {
                ...
                "uri": link["uri"],
                "fullStatement": title,
                "humanCodingScheme": f"miscon:{domain}",
    ...
    for r in records:
        ...
        item = {
            ...
            "humanCodingScheme": f"miscon:{r['id']}",
        ...
        if specializes:
            ...
            assoc("isChildOf", link, parent, f"isChildOf/{r['id']}/{specializes[0]}", notes="miscon relation: specializes")
            for extra in specializes[1:]:
                assoc("isRelatedTo", link, record_links[extra], f"isRelatedTo/{r['id']}/specializes/{extra}", notes="miscon relation: specializes (secondary parent)")
        else:
                ...
                if isinstance(t, str) and t in record_links:
                    ...
                    for a, b in ((r["id"], t), (t, r["id"])):
                        assoc(
                            ...
                            notes=f"miscon relation: {rel_name}",
                        )
