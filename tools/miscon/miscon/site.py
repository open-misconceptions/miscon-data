...
Output layout (GitHub Pages serves `m/<id>.html` at `/m/<id>`):
    ...
    schema/*.schema.json
    miscon.case.json, miscon.jsonl, miscon.csv
...
<body>
<header>
<nav aria-label="Site"><a href="{config.base_uri}/">{esc(config.title)}</a> <a href="{config.base_uri}/records.html">Records</a> <a href="{config.base_uri}/schema/">Schema</a> <a href="https://github.com/open-misconceptions/miscon-data">GitHub</a></nav>
</header>
<main id="main">
{body}
</main>
...
<p><a href="https://github.com/open-misconceptions/miscon-data/blob/main/GOVERNANCE.md">Governance</a> · <a href="https://github.com/open-misconceptions/miscon-data/blob/main/CONTRIBUTING.md">How to propose a record</a> · <a href="https://github.com/open-misconceptions/miscon-data">Source</a></p>
</footer>
</body>
</html>
"""
...
def render_record(config: Config, r: dict, by_id: dict[str, dict], schemes: dict[str, dict] | None = None) -> str:
    ...
    parts.append(f'<li><a href="{config.base_uri}/miscon.case.json">CASE 1.1 package</a> (CFItem <code>{esc(r["uuid"])}</code>)</li>')
    parts.append(f'<li><a href="https://github.com/open-misconceptions/miscon-data/blob/main/records/{esc(domain)}/{esc(rest)}.json">Source on GitHub</a></li>')
    parts.append("</ul>")
    parts.append(f"<h2>Cite</h2><pre><code>miscon:{esc(rid)}\n{esc(r['uri'])}</code></pre>")
...
<pre><code>miscon:math.fractions.add-across
{config.base_uri}/m/math.fractions.add-across</code></pre>
<h2>How to consume</h2>
<ul>
<li>One record: <code>{config.base_uri}/m/&lt;id&gt;.json</code></li>
<li>All records: <a href="{config.base_uri}/miscon.jsonl">JSON Lines</a> · <a href="{config.base_uri}/miscon.csv">CSV</a> · <a href="{config.base_uri}/records.json">index JSON</a></li>
<li>CASE 1.1 framework: <a href="{config.base_uri}/miscon.case.json">miscon.case.json</a></li>
<li>Schemas: <a href="{config.base_uri}/schema/">record and diagnosis</a></li>
</ul>
<h2>How to contribute</h2>
<p>One record per pull request. Read <a href="https://github.com/open-misconceptions/miscon-data/blob/main/CONTRIBUTING.md">CONTRIBUTING.md</a> for the review checklist.</p>
"""
...
def render_schema_page(config: Config, schemas: dict[str, dict]) -> str:
    parts = ["<h1>Schemas</h1><p>JSON Schema 2020-12. Field-by-field notes are in <a href='https://github.com/open-misconceptions/miscon-data/blob/main/schema/README.md'>schema/README.md</a>.</p>"]
...
def build_site(config: Config, out_dir: Path | None = None) -> Path:
    ...
    schemas = {}
    for name in ("miscon-record.schema.json", "diagnosis-record.schema.json"):
