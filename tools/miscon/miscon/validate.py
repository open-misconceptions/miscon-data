...
def load_schema(config: Config, name: str = "miscon-record.schema.json") -> Draft202012Validator:
...
def _validate_one(
    ...
    for key, target in _relation_targets(data):
        ...
        if key in MISCONCEPTION_RELATIONS:
            if not isinstance(target, str):
                report.problems.append(Problem(path, f"relations.{key}: targets must be Open Misconceptions record ids"))
    ...
    from .trust import reviewer_weights

    # aliases included: a review recorded under a reviewer's earlier handle is
    # still a review by a registered reviewer.
    registry_ids = set(reviewer_weights(reviewer_registry))
    ...
    if data.get("trust") != expected_trust:
        report.problems.append(
            Problem(path, f"trust must be {expected_trust!r} (computed from reviews[] and reviewers/registry.json; run 'miscon trust'), got {data.get('trust')!r}")
        )
