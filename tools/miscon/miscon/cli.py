"""`miscon` command-line entry point."""
...
def _cmd_rebase_uri(args: argparse.Namespace) -> int:
    ...
    for path in report.changed:
        print(f"  {path}")
    if not args.dry_run:
        print("\nNow run: miscon validate records/ --strict && miscon index && miscon export all")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="miscon", description="Open Misconceptions tooling")
    parser.add_argument("--version", action="version", version=f"miscon {__version__}")
    ...
    p.add_argument("--version", dest="version", help="release version to stamp (default: miscon.config.json library_version)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # The caller closed the pipe (`miscon ... | head`); that is not an error.
        try:
            sys.stdout.close()
        finally:
            return 0
